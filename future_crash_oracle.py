...
VERSION = "FUTURE CRASH // MONAN OS 11.3 — ROADRAGER RELEASE"
FPS = 12
...
def road_rager(memory: MachineMemory, rng: random.Random) -> None:
...
def render(
    ...
    control_items = [
        "[Q] quit", "[A] ask", "[M] memory", "[S] signals", "[T] stars",
        "[D] dream", "[R] RoadRager", "[:] console", "[P] PANIC",
        "[SPACE] oracle", "[O] Ollama", status,
    ]
    ...
    with RawTerminal():
        ...
        while True:
            ...
            if now >= next_frame:
                ...
                render(telemetry, drops, thought, fortune, event, ollama, history, rng, incident, visible_answer)
