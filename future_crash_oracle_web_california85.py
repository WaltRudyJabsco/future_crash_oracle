...
VERSION = "PACIFIC SYSTEMS RESEARCH // ORACLE TERMINAL v1985.4"
FPS = 12
...
def render(
    ...
    status = "OLLAMA: off"
    if ollama.enabled:
        status = f"OLLAMA: {'thinking' if ollama.pending else ollama.model}{' +WEB' if ollama.web_enabled else ''}"
    control_items = [
        "[Q] quit", "[A] ask", "[M] memory", "[S] signals", "[T] stars",
        "[D] dream", "[R] RoadRager", "[:] console", "[P] PANIC",
        "[SPACE] oracle", "[O] Ollama", "[B] audio " + (audio.status if audio else "OFF"), status,
    ]
